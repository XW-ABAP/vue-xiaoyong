...
const filteredData = computed(() => {
  ...
  if (filterKey) {
    ...
    data = data.filter((row) => {
      // 遍历当前行的所有键，只要有一个属性的值包含关键字，就保留该行
      return Object.keys(row).some((key) => {
        // 将属性值转为字符串并转小写，然后查找是否包含关键字
  ...
  if (key) {
    ...
    // 使用 slice() 浅拷贝数组后再排序，避免修改原始 props.data（Vue 中不应直接修改 props）
    data = data.slice().sort((a, b) => {
      // 获取比较的两行数据中，指定排序列的值
      a = a[key]
      b = b[key]
      // 比较逻辑：如果 a === b 则返回 0；如果 a > b 返回 1；否则返回 -1
